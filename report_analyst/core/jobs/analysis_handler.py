...
from .interfaces import JobHandler, JobDefinition, JobResult, JobStatus, AnalysisJobDefinition
...
class DocumentAnalysisHandler(JobHandler):
    ...
    async def _get_document_chunks(self, job: AnalysisJobDefinition):
        ...
        chunks = await source.get_chunks(
            job.document_id,
            job.parameters.get("configuration", {})
        )
    ...
    async def _analyze_question(self, question_data, chunks, job):
        ...
        try:
            ...
            context = "\n\n".join([
                ...
                for i, chunk in enumerate(chunks[:10])  # Limit to avoid token limits
            ])
